...
const TableModal = ({ table, onClose, onUpdateStatus, isUpdating = false }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeOrders = onSnapshot(ordersQuery, (snapshot) => {
      ...
      if (table.status === 'available') {
        // Masa boşsa sadece aktif siparişleri göster
        ...
        // Masa doluysa tüm siparişleri göster (son 10 sipariş)
        filteredOrders = sortedOrders.slice(0, 10);
      }
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      if (tableOrders.length > 0) {
        // Masa boş yapılıyorsa veya teslim edildi yapılıyorsa tüm aktif siparişleri tamamla
        if (newStatus === 'available' || newStatus === 'delivered') {
          for (const order of tableOrders) {
            if (order.status !== 'served' && order.status !== 'cancelled') {
              ...
              window.dispatchEvent(new CustomEvent('orderStatusUpdated', { 
                detail: { orderId: order.id, newStatus: 'served', tableId: table.id } 
              }));
            }
          }
        } else {
  ...
  const handleTableCleared = async () => {
    try {
      // Masanın durumunu 'empty' olarak güncelle
      await onUpdateStatus(table.id, 'empty');
          ...
          {showPaymentQR ? (
            ...
            <>
              ...
              <div className="bg-base-200 p-4 rounded-lg mb-6">
                <h3 className="text-lg font-bold border-b pb-2 mb-4">Aktif Siparişler</h3>
                ...
                {loadingOrders ? (
                  ...
                  <div className="text-center py-4">
                    <p className="text-base-content/70">Bu masada aktif sipariş bulunmuyor</p>
                  </div>
                ) : (
                  <>
